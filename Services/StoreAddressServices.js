const StoreAddress = require('../Models/StoreAddressModel');
const axios = require('axios');

class AddressService {
    async createStoreAddress(storeId, CoordinatesData) {
        try {
            ...
            if (!storeId) {
                return Promise.reject(new Error('storeId required'));
            }
            ...
            const newAddress = new StoreAddress({
                StoreId: storeId,
                street: street.trim(),
                city: city.trim(),
                state: state.trim(),
